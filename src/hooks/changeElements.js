...
export const useChangeElements = () => {
    ...
    const options = ['Segura?', 'Segurisima?', 'Estas completamente segura?', 'Te quiero muchisimo', 'Pero si estas segura?', 'Muchichisisisisisimo', 'Segura que no sabes?', 'Te quiero demaciado', 'No hay reembolsos', 'Todavia no?', 'Te quiero desde aca hasta la galaxia andromeda', 'Aun no???', 'Piensalo bien']
    ...
    const handleButtonYes = () => {
        ...
        setTextP('TE QUIERO MUCHIIIISIMOOOOOOO WAAAAAAAAAAA')
        confetti()
    }
